...
train_dataset_ohe_form = joblib.load(ROOT_DIR + '/samples/' + 'train_ohe_gbc.pickle')
test_dataset_ohe_form = joblib.load(ROOT_DIR + '/samples/' + 'test_ohe_gbc.pickle')
val_dataset_ohe_form = joblib.load(ROOT_DIR + '/samples/' + 'val_ohe_gbc.pickle')
...
clf = xgb.XGBClassifier(n_estimators=200, learning_rate=0.05, max_depth=4, random_state=0,tree_method='auto', reg_lambda=10)
clf.fit(train, y_train)


preds_train =  clf.predict(train_dataset_ohe_form_drop_id_target)
y_train = train_dataset_ohe_form['DEBT']
...
preds_test =  clf.predict(test_dataset_ohe_form_drop_id_target)
...
joblib.dump(clf,'./samples/GBC_best_nan.pickle')
...
preds_val =  clf.predict(val_dataset_ohe_form_drop_id_target)
...
submit_2.to_csv(ROOT_DIR + '/samples/' + 'submit_4_gbc_best_nan.csv',sep=',',index=False)
